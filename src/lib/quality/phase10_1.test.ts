 ...
 */
import { checkTaskDeduplication, generateTaskFingerprint } from "../../services/ai-agent/task-deduplicator";
...
async function runPhase101Tests() {
  ...
  const fp1 = generateTaskFingerprint("SESS-100", "Fix asset loading", "src/routes/product.$slug.tsx");
  console.log(`✅ Generated Task Fingerprint: ${fp1}`);

  const check1 = checkTaskDeduplication("SESS-100", "Fix asset loading", "src/routes/product.$slug.tsx");
  console.log(`✅ First Check: isDuplicate=${check1.isDuplicate}`);

  const check2 = checkTaskDeduplication("SESS-100", "Fix asset loading", "src/routes/product.$slug.tsx");
  console.log(`✅ Second Check (Duplicate): isDuplicate=${check2.isDuplicate} | TimeRemaining=${check2.timeRemainingSeconds}s`);
  ...
  if (!analysis.rootCause || !analysis.suggestedFix) throw new Error("❌ Build Failure Analysis test failed");
