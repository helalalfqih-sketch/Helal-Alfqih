...
export const getAdminDashboardStats = createServerFn({ method: "GET" })
  ...
  .inputValidator((raw: unknown) => z.object({ range: rangeSchema.optional() }).parse(raw ?? {}))
  .handler(
    async ({
      data,
      context,
    }): Promise<AdminDashboardStats & { selectedRange: DashboardTimeRange }> => {
      ...
      const isActive = (o: { status: string }) =>
