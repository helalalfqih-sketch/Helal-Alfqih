...
export function runInventoryUnitTests() {
  ...
  console.assert(canReserveStock(product, 5) === true, "Should allow reserving 5 items from 8 available");
  console.assert(canReserveStock(product, 9) === false, "Should reject reserving 9 items when only 8 available");
  ...
  console.assert(isLowStock(lowProduct, 5) === true, "Available 3 should trigger low stock threshold of 5");
