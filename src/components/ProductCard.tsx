...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const containerClasses =
    variant === 'horizontal'
      ? 'snap-start flex-shrink-0 w-[165px] sm:w-[190px] bg-[var(--color-surface-1)] backdrop-blur-md border border-[var(--color-border-default)] hover:border-[var(--color-primary)] hover:bg-[var(--color-surface-2)] rounded-2xl flex flex-col justify-between relative group transition-all duration-300 ease-out shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] cursor-pointer h-full overflow-hidden'
      : 'bg-[var(--color-surface-1)] backdrop-blur-md border border-[var(--color-border-default)] hover:border-[var(--color-primary)] hover:bg-[var(--color-surface-2)] rounded-2xl flex flex-col justify-between relative group transition-all duration-300 ease-out shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] w-full cursor-pointer h-full overflow-hidden';
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-[110px] sm:h-[130px] bg-[var(--color-surface-2)]/80 p-2 flex items-center justify-center overflow-hidden">
        {/* Product Image */}
        <img
          src={product.image}
          alt={product.name}
          ...
          <motion.button
            ...
            className={`w-7 h-7 flex items-center justify-center rounded-full transition-all cursor-pointer pointer-events-auto backdrop-blur-md border ${
              isFavorite
                ...
                : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] bg-[var(--color-surface-1)]/80 border-[var(--color-border-default)]'
            }`}
          >
            <Heart
      ...
      <div className="p-2.5 flex flex-col flex-1 justify-between space-y-2 text-right dir-rtl">
        {/* Product Name */}
        <div>
          <h4 className="font-bold text-xs sm:text-sm text-[var(--color-text-primary)] line-clamp-1 group-hover:text-[var(--color-primary)] transition-colors leading-tight">
            {product.name}
          </h4>
          ...
          <div className="flex items-center gap-1.5 text-[11px] mt-1 text-[var(--color-text-secondary)]">
            ...
            <span className="text-[var(--color-text-muted)] font-medium">({product.reviewsCount || 120} تقييم)</span>
          </div>
        </div>

        {/* Pricing Area */}
        <div className="space-y-0.5">
          <div className="text-[var(--color-text-primary)] font-black text-xs sm:text-sm tracking-tight">
            {formatPrice(product.priceYER, currency)}
          </div>
          {product.originalPriceYER > product.priceYER && (
            <div className="text-[var(--color-text-muted)] line-through text-[11px] font-medium">
        ...
        <motion.button
          ...
          className="w-full mt-1 bg-[#2F6BFF] hover:bg-[#2458D8] text-white text-xs font-bold py-2 px-2 rounded-xl flex items-center justify-center gap-1.5 transition-all duration-200 shadow-md shadow-blue-600/20 cursor-pointer active:scale-95 group/btn whitespace-nowrap"
        >
