...
export function AdminPanel({
  ...
  // Real Authentication state from adminAuth
  ...
  // Check active Supabase Auth session + user_roles permissions on mount
  React.useEffect(() => {
    let isMounted = true;
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await loginAdminWithCredentials(adminEmail, adminPassword);
      ...
      if (res.success && res.user) {
        ...
        setIsAuthenticated(true);
        showToast(
          `🔓 أهلاً وسهلاً بك ${res.user.name}! تم تسجيل الدخول بنجاح.`,
        );
      } else {
        setLoginError(
          res.error || "فشل تسجيل الدخول. يرجى التأكد من البريد وكلمة المرور.",
        );
      ...
      setLoginError("خطأ أثناء المصادقة: " + errorMessage);
    }
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    const res = await sendAdminPasswordReset(adminEmail);
    setIsLoggingIn(false);
    if (res.success) {
  ...
  const handleLogout = async () => {
    ...
    showToast("🔒 تم تسجيل الخروج من لوحة التحكم بنجاح");
  };
  ...
  if (isCheckingSession) {
    return (
      <div
        dir="rtl"
        className="fixed inset-0 z-[100] bg-[#060312] text-white flex items-center justify-center p-4 font-sans"
      >
  ...
  if (!isAuthenticated) {
    return (
      <div
        dir="rtl"
        className="fixed inset-0 z-[100] bg-[#060312] text-white flex items-center justify-center p-4 font-sans overflow-y-auto"
      >
        {/* Toast Notification */}
        {toastMessage && (
        ...
        <div className="bg-[#0c0824]/95 border border-[#3b1e82] rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl backdrop-blur-xl relative space-y-6">
          <div className="text-center space-y-3">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-200 via-white to-purple-400">
                تسجيل دخول الإدارة المحمية
              </h2>
              <p className="text-xs text-purple-300/80 mt-1">
                التحقق عبر Supabase Auth وصلاحيات جدول user_roles
              </p>
            </div>
          </div>
          ...
          {resetSuccessMsg && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-3 flex items-center gap-2 text-emerald-300 text-xs">
          ...
          {!forgotPasswordMode ? (
            <form onSubmit={handleLogin} className="space-y-4 text-xs">
              <div>
                <label className="block text-purple-200 font-semibold mb-1.5 flex items-center gap-1.5">
                  ...
                  <span>البريد الإلكتروني للمسؤول</span>
                </label>
                <input
                  type="email"
                  required
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••••••"
                    dir="ltr"
                    className="w-full px-4 py-3 pr-4 pl-10 rounded-xl bg-[#140b36] border border-purple-500/30 text-white text-sm placeholder-purple-300/40 focus:outline-none focus:border-purple-400 transition-all"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute inset-y-0 left-0 pl-3 flex items-center text-purple-400 hover:text-purple-200"
                  >
                    {showPassword ? (
            ...
            <form onSubmit={handleResetPassword} className="space-y-4 text-xs">
              <div>
                <label className="block text-purple-200 font-semibold mb-1.5 flex items-center gap-1.5">
                  <User className="w-3.5 h-3.5 text-purple-400" />
                  <span>بريد استعادة كلمة المرور</span>
                </label>
                <input
                  type="email"
                  required
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs"
                >
                ...
                <button
                  ...
                  className="py-3 px-4 rounded-xl bg-purple-900/40 text-purple-300 hover:bg-purple-800/40 text-xs"
                >
                  إلغاء
                </button>
              </div>
          ...
          <div className="pt-2 border-t border-[#3b1e82]/40 text-center">
            <button
              onClick={onClose}
              type="button"
  ...
  // Metrics calculation
  const totalRevenueYER = orders.reduce(
    (acc, o) => acc + (o.totalPriceYER || 0),
    0,
  );
  const processingOrdersCount = orders.filter(
    (o) => o.status === "processing",
  ).length;
  const shippingOrdersCount = orders.filter(
    (o) => o.status === "shipped",
  ).length;
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      const prodToSave = {
        ...
        originalPriceYER:
          Number(editingProduct.originalPriceYER) ||
          Number(editingProduct.priceYER) ||
          0,
      ...
      await saveSupabaseProduct(prodToSave);
      showToast("✅ تم حفظ المنتج في سوبا بيس بنجاح!");
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (
      !window.confirm(
        "هل أنت تأكد من حذف هذا المنتج نهائياً من قاعدة البيانات؟",
      )
    )
      return;
    try {
      await deleteSupabaseProduct(productId);
      showToast("🗑️ تم حذف المنتج من سوبا بيس");
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateOrderStatus = async (
    ...
    try {
      await updateSupabaseOrderStatus(orderId, status, statusLabel);
      showToast("⚡ تم تحديث حالة الطلب فوراً في سوبا بيس");
    } catch (err) {
      console.error(err);
  ...
  const handleResetCatalog = async () => {
    if (
      !window.confirm(
        "هل تريد استيراد وإعادة المزامنة مع الكتالوج الأساسي في الفايربيس؟",
      )
    )
      return;
    try {
      await seedInitialProductsIfNeeded();
      showToast("🔄 تم تحديث المنتجات في سوبا بيس بنجاح!");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div
      dir="rtl"
      className="fixed inset-0 z-[100] bg-[#060312] text-white overflow-y-auto flex flex-col font-sans"
    >
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-[100] bg-purple-600 text-white px-6 py-3 rounded-full shadow-2xl border border-purple-300/40 flex items-center gap-2 animate-bounce">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-purple-300/70">
              مستودع Indexes Store • قاعدة بيانات Supabase الحية
            </p>
          </div>
        </div>

        {/* Header Actions & Logged In User Info */}
        <div className="flex flex-wrap items-center gap-3">
          {currentAdmin && (
            <div className="hidden md:flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-purple-950/60 border border-purple-500/30 text-xs">
              <div className="w-6 h-6 rounded-full bg-indigo-500/30 border border-indigo-400/30 flex items-center justify-center text-indigo-300 font-bold text-[10px]">
                👑
              </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-rose-950/40 hover:bg-rose-900/60 border border-rose-500/30 text-rose-200 text-xs sm:text-sm font-medium transition-all duration-200 hover:scale-105 cursor-pointer"
      ...
      <div className="bg-[#0f0a2e] border-b border-[#3b1e82]/40 px-4 sm:px-8 py-2 overflow-x-auto no-scrollbar">
        <div className="flex items-center gap-2 min-w-max">
          {[
            ...
            {
              id: "products",
              label: `إدارة المنتجات (${products.length})`,
              icon: Package,
            },
            { id: "inventory", label: "المخزون والكميات", icon: Boxes },
            {
              id: "orders",
              label: `الطلبات (${orders.length})`,
              icon: ShoppingBag,
            },
            ...
            return (
              ...
              >
                <Icon
                  className={`w-4 h-4 ${active ? "text-white" : "text-purple-400"}`}
                />
                <span>{tab.label}</span>
              </button>
            );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-[#0c0824]/80 backdrop-blur-md border border-[#3b1e82]/50 rounded-2xl p-5 relative overflow-hidden group">
                ...
                <p className="text-xs text-amber-300/80 font-medium">
                  {processingOrdersCount} قيد المعالجة • {shippingOrdersCount}{" "}
                  جارِ الشحن
                </p>
              </div>
              ...
              <div className="bg-[#0c0824]/80 backdrop-blur-md border border-[#3b1e82]/50 rounded-2xl p-5 relative overflow-hidden group">
                ...
                <div className="text-2xl font-bold text-white mb-1">
                  {totalProductsCount}{" "}
                  <span className="text-sm text-purple-300/70">منتج</span>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-[#0c0824]/80 border border-[#3b1e82]/50 rounded-2xl p-6 space-y-4">
                ...
                {orders.length === 0 ? (
                  <div className="text-center py-8 text-purple-300/60 text-sm">
                    لا توجد طلبات مسجلة حتى الآن. يمكنك تنفيذ طلب تجريبي من
                    المتجر!
                  </div>
                ) : (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-purple-300/70 mt-1">
                            {order.governorate} • {order.items.length} منتجات •{" "}
                            {order.date}
                          </p>
                        </div>
              ...
              <div className="bg-[#0c0824]/80 border border-[#3b1e82]/50 rounded-2xl p-6 space-y-4">
                ...
                <div className="bg-[#140b36]/80 rounded-xl p-4 border border-purple-500/30 space-y-3 text-xs">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-purple-300">حالة الربط:</span>
                ...
                <div className="pt-2 space-y-2">
                  ...
                  >
                    ...
                    <span>إضافة منتج جديد لـ Supabase</span>
                  </button>
                  ...
                  <tbody className="divide-y divide-[#3b1e82]/30 text-purple-100">
                    {filteredProducts.length === 0 ? (
                      <tr>
                        <td
                          colSpan={6}
                          className="text-center py-10 text-purple-300/60"
                        >
                      ...
                      filteredProducts.map((p) => (
                        <tr
                          key={p.id}
                          className="hover:bg-[#140b36]/50 transition-all"
                        >
                          ...
                          <td className="p-3 text-center">
                            <button
                              onClick={async () => {
                                ...
                                await saveSupabaseProduct(updated as Product);
                                showToast(
                                  `تم تغيير حالة ${product.name} بنجاح`,
                                );
                                setIsSaving(false);
                              }}
              ...
              <div className="bg-[#140b36]/90 border border-purple-500/30 rounded-2xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                  ...
                  <select
                    value={newCouponType}
                    onChange={(e) =>
                      setNewCouponType(e.target.value as "percent" | "fixed")
                    }
                    className="px-3 py-2 rounded-xl bg-[#0a0520] border border-purple-500/30 text-white focus:outline-none"
                  >
                      ...
                      orders.map((order) => (
                        ...
                        >
                          ...
                          <td className="p-3 text-purple-200">
                            {order.governorate || "صنعاء"} -{" "}
                            {order.address || "الرئيسي"}
                          </td>
            ...
            <div className="bg-[#0c0824]/80 border border-[#3b1e82]/50 rounded-2xl p-6 space-y-6">
              <div>
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  <span>إعدادات المتجر وقاعدة بيانات Supabase</span>
                </h3>
                <p className="text-xs text-purple-300/70 mt-1">
                  المشروع مربوط بشكل كامل مع مستودع Indexes Store وقاعدة بيانات
                  Supabase
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-[#140b36]/80 border border-purple-500/30 rounded-2xl p-5 space-y-3">
                  ...
                  <div className="space-y-2 text-xs text-purple-200">
                    <div className="flex justify-between">
                      <span className="text-purple-300">سلسلة الاتصال:</span>
                      <span className="font-mono text-purple-200">
                        VITE_SUPABASE_URL
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-purple-300">المزود:</span>
                      <span className="font-mono text-purple-200">
                        Supabase Realtime Client
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-purple-300">التزامن الآلي:</span>
          ...
          <div className="bg-[#0c0824] border border-[#3b1e82] rounded-3xl p-6 max-w-xl w-full space-y-4 my-8 text-right relative shadow-2xl">
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingProduct({
                        ...editingProduct,
                        priceYER: Number(e.target.value),
                      })
                    }
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-[#3b1e82]/50">
                ...
                >
                  {isSaving ? "جاري الحفظ..." : "حفظ المنتج في Supabase"}
                </button>
              </div>
            </form>
          </div>
