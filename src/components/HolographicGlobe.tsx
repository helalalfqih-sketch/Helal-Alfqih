...
export const HolographicGlobe: React.FC<HolographicGlobeProps> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // 1. Draw Core Radial Atmosphere Glow with HDR Depth & Controlled Blue Rim
      const bgGlow = ctx.createRadialGradient(cx, cy, radius * 0.15, cx, cy, radius * 1.35);
      bgGlow.addColorStop(0, 'rgba(47, 107, 255, 0.15)');
      bgGlow.addColorStop(0.5, 'rgba(56, 189, 248, 0.10)');
      bgGlow.addColorStop(0.85, 'rgba(14, 165, 233, 0.04)');
      ...
      // Outer Sphere Blue/Cyan Rim Light
      ctx.beginPath();
      ...
      ringGrad.addColorStop(0, 'rgba(47, 107, 255, 0.7)');
      ringGrad.addColorStop(0.5, 'rgba(56, 189, 248, 0.85)');
      ringGrad.addColorStop(1, 'rgba(47, 107, 255, 0.7)');
      ...
      // Orbiting particles on the ring
      ...
      for (let p = 0; p < 4; p++) {
        ...
        ctx.fillStyle = p % 2 === 0 ? '#38bdf8' : '#60a5fa';
        ctx.shadowColor = '#38bdf8';
        ctx.shadowBlur = 6;
        ctx.fill();
      }
      ...
      longitudes.forEach((lon) => {
        ...
        ctx.strokeStyle = isFrontMeridian
      ...
      // 5. Draw Grid Intersection Glowing Node Dots
      latitudes.forEach((lat) => {
        longitudes.forEach((lon) => {
          const { px, py, z } = project(lat, lon);
          if (z > -radius * 0.1) {
      ...
      // 6. Calculate Product Badge Positions
  ...
  // Touch and Mouse Handlers for Dragging ("اسحب الكرة")
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    // Clamp pitch angle so it doesn't flip completely upside down
    ...
    >
      ...
      {showTitleBadge && (
        <div className="absolute -top-7 sm:-top-8 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1.5 bg-[var(--color-surface-2)] border border-[var(--color-border-default)] px-3 py-1 rounded-full text-[10px] sm:text-[11.5px] font-extrabold text-[var(--color-text-primary)] shadow-md whitespace-nowrap backdrop-blur-md">
          <span className="material-symbols-outlined text-[#2F6BFF] text-[15px]">
            3d_rotation
          </span>
      ...
      {/* Interactive Overlay Product Badges Floating on 3D Surface */}
