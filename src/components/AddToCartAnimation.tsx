        ...
        {activeFlyingItems.map((item) => {
          ...
          return (
            <React.Fragment key={item.id}>
              ...
              >
                <img
                  src={item.product.image || getSmartProductFallbackImage(item.product.name)}
                  alt={item.product.name}
                  onError={(e) => {
                    e.currentTarget.src = getSmartProductFallbackImage(item.product.name);
                  }}
          ...
          >
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="relative w-12 h-12 rounded-xl bg-[var(--color-surface-2)] p-1 border border-[var(--color-border-default)] shrink-0 overflow-hidden">
                <img
                  src={currentToast.product.image || getSmartProductFallbackImage(currentToast.product.name)}
                  alt={currentToast.product.name}
                  onError={(e) => {
                    e.currentTarget.src = getSmartProductFallbackImage(currentToast.product.name);
                  }}
