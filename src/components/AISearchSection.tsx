...
import { Bot, Sparkles, Search, Headphones, Radio, Camera, Watch, Gamepad2, Zap } from 'lucide-react';
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              {aiResult.matchedProducts.map((p) => (
                <div
                  ...
                  className="bg-[var(--color-surface-2)] hover:bg-[var(--color-surface-3)] border border-[var(--color-border-default)] hover:border-[var(--color-primary)] p-2.5 rounded-2xl flex items-center gap-2.5 cursor-pointer transition-all group backdrop-blur-md shadow-sm"
                >
                  <img
                    src={p.image}
                    alt={p.name}
                    className="w-10 h-10 rounded-xl object-cover shrink-0 border border-[var(--color-border-subtle)]"
                  />
                  <div className="overflow-hidden text-right">
                    <h5 className="text-xs font-bold text-[var(--color-text-primary)] truncate group-hover:text-[var(--color-primary)] transition-colors">
                      {p.name}
                    </h5>
