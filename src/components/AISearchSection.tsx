...
import { Bot, Sparkles, Search, Headphones, Radio, Camera, Watch, Gamepad2, Zap, Tag, ArrowLeft, History, X } from 'lucide-react';
import { getRecentSearches, saveRecentSearch, removeRecentSearch, clearRecentSearches } from '../lib/searchHistory';

import { getSmartProductFallbackImage } from '../lib/product-fallback-image';
...
// Helper function to highlight query match in product names and categories
...
export const AISearchSection: React.FC<AISearchSectionProps> = ({
  ...
  // Smart Autocomplete Suggestions Calculation
  const suggestions = useMemo(() => {
    if (!trimmedQuery) return { products: [], categories: [], totalMatchesCount: 0 };
    ...
    // Find matching products (title, subtitle, category, description)
    const matchingProducts = products.filter((p) => {
      const nameMatch = p.name.toLowerCase().includes(trimmedQuery);
      const categoryMatch = p.category ? p.category.toLowerCase().includes(trimmedQuery) : false;
      const subtitleMatch = p.subtitle ? p.subtitle.toLowerCase().includes(trimmedQuery) : false;
      const descMatch = p.description ? p.description.toLowerCase().includes(trimmedQuery) : false;
      return nameMatch || categoryMatch || subtitleMatch || descMatch;
    });

    // Sort matching products so title matches come first
    matchingProducts.sort((a, b) => {
      const aTitle = a.name.toLowerCase().includes(trimmedQuery) ? 0 : 1;
      const bTitle = b.name.toLowerCase().includes(trimmedQuery) ? 0 : 1;
    ...
    return {
      ...
      totalMatchesCount: matchingProducts.length,
    };
  }, [trimmedQuery]);
  ...
  // Keyboard navigation handler for autocomplete dropdown
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (!showSuggestions) return;

    const totalItems = suggestions.products.length;
            ...
            <div className="relative flex items-center bg-[var(--color-surface-2)] border border-[var(--color-border-default)] focus-within:border-[var(--color-primary)] rounded-full p-1 shadow-inner backdrop-blur-md">
              ...
              {query.length > 0 && (
                <button
                  ...
                  className="pl-1 pr-1 text-[var(--color-text-muted)] hover:text-rose-400 transition-colors"
                  title="مسح النص"
                >
                  <X className="w-4 h-4" />
                </button>
          ...
          <AnimatePresence>
            {showSuggestions && (trimmedQuery.length > 0 || recentSearches.length > 0) && (
              <motion.div
                ...
                className="absolute top-full left-0 right-0 mt-2 z-50 bg-[var(--color-surface-1)] border border-[var(--color-border-default)] rounded-2xl shadow-2xl p-3 backdrop-blur-2xl dir-rtl overflow-hidden text-right max-h-[380px] overflow-y-auto space-y-3"
              >
                ...
                {suggestions.categories.length > 0 && (
                  <div className="pb-2.5 border-b border-[var(--color-border-subtle)]">
                    <div className="flex items-center gap-1.5 text-[11px] font-bold text-[#2F6BFF] mb-2">
                      ...
                      <span>الأقسام والتصنيفات المتطابقة</span>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {suggestions.categories.map((cat, idx) => (
                        ...
                        >
                          <Tag className="w-3 h-3 text-blue-400 opacity-75" />
                          <span>{highlightMatch(cat, trimmedQuery)}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {suggestions.products.length > 0 ? (
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-[11px] font-bold text-[var(--color-text-muted)] px-1 mb-1">
                      <span>
                        اقتراحات عناوين المنتجات ({suggestions.products.length} من {suggestions.totalMatchesCount})
                      </span>
                      <span className="text-[10px] text-[#2F6BFF]">استخدم الأسهم ↕ للتنقل أو اضغط للاستعراض</span>
                    </div>
                      ...
                      return (
                        <div
                          ...
                          className={`group/item flex items-center justify-between gap-3 p-2 rounded-xl transition-all cursor-pointer border ${
                            isKeyboardSelected
                              ? 'bg-[var(--color-surface-3)] border-[#2F6BFF] ring-1 ring-[#2F6BFF]'
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            <img
                              ...
                              className="w-10 h-10 rounded-lg object-contain bg-[var(--color-surface-2)] p-0.5 shrink-0 border border-[var(--color-border-subtle)]"
                            />
                            <div className="min-w-0 text-right">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[10px] text-[#2F6BFF] bg-[#2F6BFF]/10 px-1.5 py-0.2 rounded font-semibold shrink-0">
                                  {highlightMatch(prod.category, trimmedQuery)}
                                </span>
                                {prod.subtitle && (
                ...
                ) : trimmedQuery.length > 0 && suggestions.categories.length === 0 ? (
                  <div className="p-3 text-center text-xs text-[var(--color-text-muted)] font-medium">
                    لا توجد منتجات مطابقة لـ "{query}". اضغط بحث لطلب نتائج إضافية من الذكاء الاصطناعي.
                  </div>
                ) : null}

                {/* Recent Searches dropdown block if query is empty */}
                {trimmedQuery.length === 0 && recentSearches.length > 0 && (
                  <div className="pt-1">
                    <div className="flex items-center justify-between text-[11px] font-bold text-[var(--color-text-muted)] mb-2 px-1">
                      <span className="flex items-center gap-1">
                        <History className="w-3.5 h-3.5 text-[#2F6BFF]" />
                        <span>سجل عمليات البحث السابقة</span>
                      </span>
                      <button
                        type="button"
                        onClick={handleClearRecentSearches}
                    ...
                    <div className="space-y-1">
                      {recentSearches.map((term, index) => (
                        <div
                          ...
                          className="flex items-center justify-between p-2 rounded-xl hover:bg-[var(--color-surface-2)] text-xs text-[var(--color-text-primary)] font-semibold cursor-pointer transition-colors"
                        >
                          ...
                          <button
                            type="button"
                            onClick={(e) => handleRemoveRecentSearch(e, term)}
                            className="text-[var(--color-text-muted)] hover:text-rose-400 p-1 rounded-full transition-colors"
                          >
