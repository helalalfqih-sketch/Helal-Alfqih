...
export const OrderSelfServiceModal: React.FC<OrderSelfServiceModalProps> = ({
  ...
  const [activeTab, setActiveTab] =
    useState<ChangeRequestType>("change_address");

  // Form states
  const [newGovernorate, setNewGovernorate] = useState(
    order?.governorate || YEMEN_GOVERNORATES[0],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await submitOrderChangeRequest(
      order.id,
      order.orderNumber,
      activeTab,
      details,
      ...
      if (onOrderUpdated) {
        const updatedOrder: OrderStatus = {
          ...
          ...(activeTab === "change_phone" && {
            phone: newPhone,
            customerName: newRecipientName,
          }),
          ...(activeTab === "cancel_request" && {
            status: "cancelled",
            statusLabel: "تم إلغاء الطلب بناءً على طلبك ❌",
          }),
        };
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--color-text-secondary)]">
                  خدمات التعديل والتحكم المباشر في الطلب
                </p>
              </div>
            </div>
          ...
          {!editable ? (
            <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs space-y-2">
              ...
              <p>
                حالة الطلب حالياً هي &quot;{order.statusLabel}&quot;. نظرًا لأن
                الشحنة خرجت للتسليم أو جاري شحنها، لا يمكن تعديل البيانات
                تلقائياً. يمكنك التواصل فوراً مع خدمة العملاء للتعديل.
              </p>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {activeTab === "change_address" && (
                  <div className="space-y-3">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">
                        العنوان الجديد والشارع وأقرب معلم
                      </label>
                      <textarea
                        required
                        rows={3}
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-2 p-3 rounded-xl bg-[var(--color-surface-2)] border border-[var(--color-border-default)] cursor-pointer">
                      ...
                      <span className="text-xs font-bold text-[var(--color-text-primary)]">
                        الاتصال بي هاتفياً قبل الوصول لتأكيد التواجد
                      </span>
                    </label>

                    <div>
                      <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">
                        تعليمات إضافية للمندوب
                      </label>
                      <textarea
                        rows={3}
                        value={notes}
                ...
                {activeTab === "cancel_request" && (
                  <div className="space-y-3">
                    <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs">
                      سيؤدي هذا إلى تقديم طلب إلغاء فوري للطلب #
                      {order.orderNumber}.
                    </div>
