...
import { 
  ...
  Sparkle, CheckCircle2, Gift, Zap, DollarSign, Package, Globe
} from 'lucide-react';
...
// Helper to verify WebGL capability before rendering 3D canvas
...
export const ProductUniverseModal: React.FC<ProductUniverseModalProps> = ({
  ...
  const [isWebGLAvailable, setIsWebGLAvailable] = useState<boolean>(() => checkWebGLSupport());

  // Handle modal opening and WebGL capability check
  useEffect(() => {
    if (isOpen) {
      const webglSupported = checkWebGLSupport();
      setIsWebGLAvailable(webglSupported);
      logUniverseEvent('product_universe_opened', { totalProducts: products.length, webglSupported });
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[150] bg-[#03010b] text-white flex flex-col justify-between overflow-hidden dir-rtl select-none font-sans">
        ...
        <div className="relative flex-1 w-full h-full overflow-hidden">
          ...
          {/* 3D Holographic Sphere Globe View */}
          <div className="w-full h-full relative z-10 bg-[#03010b] flex items-center justify-center overflow-hidden">
            <WebGLErrorBoundary
              fallback={
                /* CSS 3D Animated Sphere Globe Fallback */
                <div className="relative w-full h-full flex items-center justify-center bg-[#03010b] overflow-hidden">
                  {/* Glowing Holographic Sphere Core */}
                  <div className="relative w-64 h-64 sm:w-80 sm:h-80 rounded-full bg-gradient-to-tr from-cyan-500/20 via-purple-600/30 to-pink-500/20 border border-cyan-500/40 shadow-[0_0_80px_rgba(0,240,255,0.4)] flex items-center justify-center animate-pulse">
                    <div className="absolute inset-2 rounded-full border border-purple-400/30 animate-spin" style={{ animationDuration: '15s' }} />
                    <div className="absolute inset-6 rounded-full border border-pink-500/30 animate-spin" style={{ animationDuration: '25s', animationDirection: 'reverse' }} />
                    <Globe className="w-24 h-24 text-cyan-400 opacity-40 animate-pulse" />
                    <div className="absolute text-center z-10 px-4">
                      <span className="text-xs font-black text-white block">كرة عالم المنتجات 🌎</span>
                      <span className="text-[10px] text-cyan-300">استكشف المنتجات المدارية</span>
                    </div>
                  </div>

                  {/* Spherical Orbiting Product Floating Nodes */}
                  <div className="absolute inset-0 p-4 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 z-20 pointer-events-auto bg-black/40 backdrop-blur-xs">
                    {filteredProducts.map((prod) => (
                      <div
                        key={prod.id}
                        onClick={() => handleSelectProduct(prod)}
                        className="bg-[#0e0a24]/90 border border-cyan-500/30 hover:border-cyan-400 rounded-2xl p-3 flex flex-col justify-between shadow-[0_0_15px_rgba(0,240,255,0.15)] transition-all cursor-pointer hover:scale-105"
                      >
            ...
            >
                ...
                onCreated={({ gl }) => {
                  ...
                  if (canvas) {
                    canvas.addEventListener('webglcontextlost', (e) => {
                      e.preventDefault();
                      console.warn('WebGL context lost, maintaining 3D sphere fallback');
              ...
              >
                <Suspense fallback={
                    <Html center>
                      <div className="bg-[#0e0a26]/90 border border-purple-500/30 text-purple-300 text-xs font-bold px-4 py-2 rounded-2xl backdrop-blur-md animate-pulse">
                        جاري تجهيز عالم المنتجات...
                      </div>
                    </Html>
                  }>
                ...
                <div className="bg-[#0e0a26] border border-purple-500/40 rounded-3xl p-5 max-w-sm w-full relative text-right">
                  ...
                  {activeTabModal === 'demo' && (
                    <div>
                      ...
                      <img src={focusedProduct.image || FALLBACK_IMAGE} alt="" onError={handleImageError} className="w-full h-36 object-contain bg-black/40 rounded-2xl p-2 mb-3 border border-white/10" />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {compareList.map((prod) => (
                        <div key={prod.id} className="bg-black/40 border border-white/10 rounded-2xl p-3 flex flex-col justify-between">
                          <div>
                            <img src={prod.image || FALLBACK_IMAGE} alt="" onError={handleImageError} className="w-full h-24 object-contain mb-2" />
                            <h4 className="text-xs font-bold text-white line-clamp-1">{prod.name}</h4>
