      ...
      <div className="w-full max-w-md bg-[var(--color-surface-1)] border-r border-[var(--color-border-default)] h-full flex flex-col justify-between p-6 shadow-2xl relative overflow-y-auto no-scrollbar">
        ...
        <div>
          <div className="flex items-center justify-between pb-4 border-b border-[var(--color-border-default)] mb-4">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[28px] text-[#2F6BFF]">
                shopping_cart
              </span>
              <h3 className="text-xl font-bold text-[var(--color-text-primary)]">سلة التسوق</h3>
              <span className="bg-[#2F6BFF]/15 text-[#2F6BFF] text-xs font-bold px-2.5 py-0.5 rounded-full border border-[#2F6BFF]/30">
          ...
          <div className="bg-[var(--color-surface-2)] p-4 rounded-2xl border border-[var(--color-border-default)] mb-5">
            <div className="flex justify-between items-center text-xs text-[var(--color-text-secondary)] font-semibold mb-2">
              <span>
                {isFreeShipping ? (
            ...
            <div className="w-full bg-[var(--color-surface-3)] h-2.5 rounded-full overflow-hidden">
              <div
                className="bg-gradient-to-r from-[#2F6BFF] to-[#3B82F6] h-full transition-all duration-500 rounded-full"
          ...
          {cartItems.length === 0 ? (
            <div className="py-12 text-center text-[var(--color-text-muted)] flex flex-col items-center gap-3">
              <span className="material-symbols-outlined text-[64px] text-[var(--color-text-muted)]">
                remove_shopping_cart
              </span>
              <p className="text-base font-medium">سلة التسوق فارغة حالياً</p>
              <button
                onClick={onClose}
                className="mt-2 text-[#2F6BFF] font-bold text-sm hover:underline"
              >
                تصفح المنتجات الآن
              </button>
            </div>
            ...
            <div className="space-y-4 max-h-[380px] overflow-y-auto no-scrollbar pr-1">
              {cartItems.map((item) => (
                <div
                  key={item.product.id}
                  className="bg-[var(--color-surface-2)] p-3.5 rounded-2xl border border-[var(--color-border-subtle)] flex items-center gap-3 relative group"
                >
                  {/* Thumbnail */}
                  <img
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <span className="text-[var(--color-text-primary)] font-bold text-sm block mt-1">
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <button
                      ...
                      className="text-[var(--color-text-muted)] hover:text-rose-500 transition-colors p-1"
                      aria-label="حذف"
                    >
                      <span className="material-symbols-outlined text-[18px]">delete</span>
                    </button>

                    <div className="flex items-center gap-2 bg-[var(--color-surface-3)] border border-[var(--color-border-default)] px-2 py-1 rounded-xl">
                      <button
                        onClick={() =>
                          onUpdateQuantity(item.product.id, Math.max(1, item.quantity - 1))
                        }
                        className="text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] font-bold text-xs px-1"
                      >
                        -
                      </button>
                      <span className="text-[var(--color-text-primary)] font-bold text-xs">{item.quantity}</span>
                      <button
        ...
        {cartItems.length > 0 && (
          <div className="pt-4 border-t border-[var(--color-border-default)] space-y-3 mt-4">
            ...
            <div>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-grow bg-[var(--color-surface-2)] border border-[var(--color-border-default)] rounded-xl px-3 py-2 text-xs text-[var(--color-text-primary)] uppercase placeholder-[var(--color-text-muted)] focus:border-[#2F6BFF] outline-none"
                />
                <button
                  onClick={applyCoupon}
                  className="bg-[var(--color-surface-3)] hover:bg-[#2F6BFF] border border-[var(--color-border-default)] text-[var(--color-text-primary)] hover:text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors cursor-pointer shrink-0"
                >
                  تطبيق
                </button>
              </div>
            ...
            {/* Price Calculations */}
            <div className="space-y-1.5 text-xs text-[var(--color-text-secondary)] pt-2 border-t border-[var(--color-border-subtle)]">
              <div className="flex justify-between">
                <span>المجموع الفرعي:</span>
                <span className="font-semibold text-[var(--color-text-primary)]">{formatPrice(subtotalYER, currency)}</span>
              </div>
              ...
              <div className="flex justify-between text-base font-extrabold text-[var(--color-text-primary)] pt-2 border-t border-[var(--color-border-default)]">
            ...
            <button
              ...
              className="w-full bg-[#2F6BFF] hover:bg-[#2458D8] text-white font-bold py-3.5 rounded-2xl shadow-xl shadow-blue-500/20 flex items-center justify-center gap-2 transition-all active:scale-95 text-base cursor-pointer"
            >
