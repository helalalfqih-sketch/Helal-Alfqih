...
export const ProductCompareModal: React.FC<ProductCompareModalProps> = ({
  ...
  const displayedSpecKeys = useMemo(() => {
    if (!showOnlyDiffs) return allSpecKeys;
    return allSpecKeys.filter((key) => isSpecDifferent(key));
  }, [allSpecKeys, showOnlyDiffs, compareList]);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-base sm:text-lg font-extrabold text-white">مقارنة المنتجات والمواصفات الفنية</h2>
                ...
                {compareList.length > 1 && (
                  <>
                    ...
                    >
                      ...
                      <span>{showOnlyDiffs ? 'عرض الكل' : 'الااختلافات فقط'}</span>
                    </button>
                  </>
                )}
              ...
              {compareList.length === 0 ? (
                <div className="py-16 text-center space-y-4">
                  ...
                  <p className="text-xs text-[#A7ADB7] max-w-sm mx-auto">
                    يمكنك إضافة أي منتج إلى قائمة المقارنة بالنقر على زر المقارنة من تفاصيل أو كروت المنتجات.
                  </p>
                  ...
                  {availableToAdd.length > 0 && (
                    <div className="pt-2 max-w-xs mx-auto">
                      ...
                      >
                        <option value="" disabled>+ أضف منتجاً لبدء المقارنة...</option>
                  ...
                  <table className="w-full text-right border-collapse min-w-[700px]">
                    ...
                    <tbody>
                      ...
                      <tr className="bg-white/5 border-y border-white/10">
                        <td colSpan={compareList.length + 1} className="px-4 py-2 text-xs font-bold text-blue-400 bg-[#181d26]">
                      ...
                      {(!showOnlyDiffs || isPriceDifferent) && (
                        <tr className={`border-b border-white/5 hover:bg-white/[0.02] transition-colors ${
                          ...
                          {compareList.map((product) => (
                            <td key={`price-${product.id}`} className="p-3 text-center border-l border-white/5">
                      ...
                      {(!showOnlyDiffs || isRatingDifferent) && (
                        <tr className={`border-b border-white/5 hover:bg-white/[0.02] transition-colors ${
                          ...
                          {compareList.map((product) => (
                            <td key={`rating-${product.id}`} className="p-3 text-center border-l border-white/5">
                      ...
                      {(!showOnlyDiffs || isStockDifferent) && (
                        <tr className={`border-b border-white/5 hover:bg-white/[0.02] transition-colors ${
                          ...
                          {compareList.map((product) => (
                            <td key={`stock-${product.id}`} className="p-3 text-center border-l border-white/5">
                              <span className={`text-[11px] font-bold inline-flex items-center gap-1 px-2.5 py-1 rounded-full ${
                      ...
                      {(!showOnlyDiffs || isCategoryDifferent) && (
                        <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                          ...
                          {compareList.map((product) => (
                            <td key={`cat-${product.id}`} className="p-3 text-center border-l border-white/5 text-xs text-white/90">
                      ...
                      <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                        ...
                        {compareList.map((product) => (
                          <td key={`colors-${product.id}`} className="p-3 text-center border-l border-white/5">
                      ...
                      <tr className="bg-white/5 border-y border-white/10">
                        <td colSpan={compareList.length + 1} className="px-4 py-2 text-xs font-bold text-amber-400 bg-[#181d26]">
                      ...
                      {displayedSpecKeys.length === 0 ? (
                        <tr>
                          <td colSpan={compareList.length + 1} className="p-6 text-center text-xs text-[#717784]">
                            {showOnlyDiffs
                              ...
                              <td className="p-3 text-xs font-bold text-white/90 sticky right-0 z-20 bg-[#12151a] border-l border-white/10">
                                <div className="flex items-center justify-between gap-1">
                                  ...
                                  {highlightDiffs && isDiff && (
                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0" title="يوجد اختلاف في هذه الخاصية" />
                                  )}
                                </div>
                              </td>
                              {compareList.map((product) => {
                                ...
                                return (
                                  <td key={`spec-${specKey}-${product.id}`} className="p-3 text-center border-l border-white/5 text-xs">
                                    {specVal ? (
                                      <span className={`font-semibold ${isDiff && highlightDiffs ? 'text-blue-300 font-bold' : 'text-white/80'}`}>
                                        {specVal}
                                      </span>
                                    ) : (
                                      <span className="text-[#717784] text-[11px]">— غير محدد —</span>
                                    )}
                                  </td>
                                );
                              })}
                      ...
                      <tr className="bg-white/5 border-y border-white/10">
                        <td colSpan={compareList.length + 1} className="px-4 py-2 text-xs font-bold text-[#A7ADB7] bg-[#181d26]">
                          📝 الوصف والملخص
                        </td>
                      </tr>
                      <tr className="border-b border-white/5">
                        ...
                        {compareList.map((product) => (
                          <td key={`desc-${product.id}`} className="p-3 text-right border-l border-white/5 text-xs text-[#A7ADB7] leading-relaxed">
