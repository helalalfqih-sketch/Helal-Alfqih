        ...
        >
          ...
          <div className="flex items-center justify-between pb-4 border-b border-[var(--color-border-default)] mb-5 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-extrabold text-[var(--color-text-primary)]">دفتر عناوين اليمن الذكي</h3>
                <p className="text-xs text-[var(--color-text-secondary)]">إدارة عناوين الشحن والتوصيل المباشر</p>
              </div>
            </div>
          ...
          {!isAddingNew ? (
            <div className="space-y-4 flex-1">
              ...
              {addresses.length === 0 ? (
                <div className="p-8 text-center rounded-2xl border border-dashed border-[var(--color-border-default)] bg-[var(--color-surface-2)] space-y-3">
                  ...
                  <div className="text-sm font-bold text-[var(--color-text-primary)]">لا توجد عناوين محفوظة حالياً</div>
                  <p className="text-xs text-[var(--color-text-muted)] max-w-sm mx-auto">
                    أضف عنوانك المفصل في أي محافظة يمنية لتسريع عملية الطلب والتوصيل إلى باب بيتك.
                  </p>
                  <button
                    onClick={() => setIsAddingNew(true)}
                ...
                <div className="space-y-3">
                  {addresses.map((addr) => {
                    const shipping = calculateYemenShipping(addr.governorate, subtotalYER);
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                              {addr.label === 'المنزل' && <Home className="w-3 h-3 inline ml-1" />}
                              {addr.label === 'العمل' && <Building className="w-3 h-3 inline ml-1" />}
                              {addr.label === 'هدية' && <Gift className="w-3 h-3 inline ml-1" />}
                              {addr.label}
                            </span>
                            <span className="font-extrabold text-sm text-[var(--color-text-primary)]">{addr.recipientName}</span>
                          </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1.5">نوع العنوان</label>
                <div className="grid grid-cols-4 gap-2">
                  {(['المنزل', 'العمل', 'هدية', 'آخر'] as YemenAddress['label'][]).map((lbl) => (
                    <button
                      type="button"
                      key={lbl}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">اسم المستلم الثلاثي *</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">رقم الهاتف الرئيسي (اليمن) *</label>
                  <div className="relative">
                    <input
                      type="tel"
                      required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">المديرية / المنطقة *</label>
                  <input
                    type="text"
                    required
                    value={district}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">الشارع</label>
                  <input
                    type="text"
                    value={street}
                ...
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">أقرب معلم بارز *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">رقم هاتف إضافي (اختياري)</label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-xs font-bold text-[var(--color-text-secondary)] mb-1">تعليمات المندوب (اختياري)</label>
                  <input
                    type="text"
                    value={deliveryNotes}
