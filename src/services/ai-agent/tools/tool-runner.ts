...
export async function executeTool(toolName: string, input: any, context: any) {
  ...
  if (!tool.allowedEnvironments.includes(currentEnv as any)) {
    throw new Error(
      `EXECUTION_ENVIRONMENT_UNAVAILABLE: Tool '${toolName}' is not allowed in environment '${currentEnv}'.`,
    );
  }
  ...
  if (db) {
    await logAudit(db, auth.tenantId, auth.userId, "tool_execution_started", context.sessionId, {
      toolName,
      input: parsedInput,
    });
  }
  ...
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(
      () => reject(new Error(`TOOL_TIMEOUT: Tool '${toolName}' exceeded ${tool.timeoutMs}ms.`)),
      tool.timeoutMs,
    ),
  );

  try {
    const result = await Promise.race([tool.execute(parsedInput, context), timeoutPromise]);
    ...
    if (db) {
      await logAudit(db, auth.tenantId, auth.userId, "tool_execution_success", context.sessionId, {
        toolName,
      });
    }
    ...
    if (db) {
      await logAudit(db, auth.tenantId, auth.userId, "tool_execution_failed", context.sessionId, {
        toolName,
        error: error.message,
      });
    }
